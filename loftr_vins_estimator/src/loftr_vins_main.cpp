...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "loftrvins_estimator");
    ros::NodeHandle n("~");
    ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Info);

    if(argc != 2)
    {
        printf("please intput: rosrun vins loftrvins_node [config file] \n"
               "for example: upervins loftrvins_node"
               "~/catkin_ws/src/loftrvins/config/euroc/euroc_mono_imu_config.yaml \n");
        return 1;
    }
    ...
    // 修正 VINS_RESULT_PATH - 確保它指向正確的完整路徑
